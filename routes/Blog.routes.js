...
blogRoutes.get("/", async (req, res) => {
  ...
  try {
    if (order && sort && title) {
      let posts = await BlogModel.find().sort(
        { date: order == "asc" ? 1 : -1 }
      );
      res.status(200).send(posts);
    } else if (order && sort && category) {
      let posts = await BlogModel.find({category
      }).sort({ date: order == "asc" ? 1 : -1 });
      res.status(200).send(posts);
    } else if (title) {
      let posts = await BlogModel.find({ title });
      res.status(200).send(posts);
    } else if (category) {
      let posts = await BlogModel.find({ category });
      res.status(200).send(posts);
